...
public class Main {
	...
	public static void main(String[] args) {
		...
		int sum = list.stream().map(Person::getPets).filter(l -> l.size() >= 3)
				.flatMap(List::stream).map(Person.Pet::getName).unordered()
				.distinct().filter(s -> s.length() >= 10)
				.flatMapToInt(String::chars).reduce(0, Integer::sum);
