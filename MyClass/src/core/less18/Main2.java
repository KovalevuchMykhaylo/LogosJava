...
public class Main2 {
	...
	public static void main(String[] args) {
		...
		artists.add(getProxySimple(new DeathLock()));
		artists.add(getProxySimple(new Rammstein()));
		artists.add(getProxySimple(new Soad()));
		Stage stage = new ArenaLviv();
		for (Artist artist : artists) {
			stage.perforanse(artist);
		}
	...
	static Artist getProxy(Artist artist) {
		...
		return (Artist) Proxy.newProxyInstance(artist.getClass()
				.getClassLoader(), artist.getClass().getInterfaces(), handler);
