...
public interface RoomServiceDao extends JpaRepository<RoomService, Long>, JpaSpecificationExecutor<RoomService>{
	...
	@Query(value="SELECT a FROM RoomService a LEFT JOIN FETCH a.hotelName LEFT JOIN FETCH a.typeOfRoom LEFT JOIN FETCH a.typeOfBathRoom",
			countQuery="SELECT count(a.id) FROM RoomService a")
