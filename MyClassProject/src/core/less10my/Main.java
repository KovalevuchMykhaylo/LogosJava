...
public class Main {
	...
	private static final Scanner sc = new Scanner(System.in);
		...
		while (true){
			...
			System.out.println("Enter 4 to sort by all");
			switch (sc.next()) {
			case "1":
		...
		static List<Person> sortByAgeAsc(){
			return List.stream().sorted(comparingInt(Person::getAge)).collect(toList());
		}
		static List<Person> sortByAgeDesc(){
			return List.stream().sorted(comparingInt(Person::getAge).reversed()).collect(toList());
		}
		static List<Person> sortByAllDesc(){
			return List.stream().sorted(comparing(Person::getName).reversed()
